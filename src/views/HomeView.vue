  ...
  <section class="font-bold font-display">
    <section class="bg-pastel-bg py-8 p-0 min-h-screen">
      <section class="place-content-center mx-auto px-4 max-w-lg container">
        <AddTodo :addTodo="addTodo" :tasks="tasks" />
        <TodoSummary :tasks="tasks" />
        <TransitionGroup name="todo">
        <section v-if="tasks.length <= 0" class="font-body text-center">
          <span>No task yet.. Add one now!</span>
          <EmptyTask />
        </section>
        <TodoItems :store="useTodoStore()" :tasks="tasks" key="todo-items"/>
      </TransitionGroup>
      </section>
    </section>
  </section>
